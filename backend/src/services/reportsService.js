...
export const getDetailedSalesReport = async (fechaInicio, fechaFin, empleadoId) => {
  ...
  for (const p of pagos || []) {
    ...
    if (p.id_metodo === efectivoId) {
      entry.efectivo += Number(p.monto);
    }
    if (p.id_metodo === qrId) {
      entry.qr += Number(p.monto);
    }
  }
  ...
  for (const e of extras || []) {
    if (!e.id_prod) {
      ...
      if (!extrasPromoByDetProd.has(dpKey)) {
        extrasPromoByDetProd.set(dpKey, new Map());
      }
      const unitMap = extrasPromoByDetProd.get(dpKey);
      if (!unitMap.has(e.num_unidad)) {
        unitMap.set(e.num_unidad, []);
      }
  ...
  for (const e of exclusionsPromo || []) {
    const dpKey = `${e.id_detalle_venta}|${e.id_prod}`;
    if (!exclusionsPromoByDetProd.has(dpKey)) {
      exclusionsPromoByDetProd.set(dpKey, new Map());
    }
  ...
  const buildGroupsForProduct = (idDetalle) => {
    ...
    if (exclusiones.length === 0 && extrasTxt.length === 0) {
      return [];
    }
  ...
  for (const d of allDetalles || []) {
    if (!detallesByVenta.has(d.id_venta)) {
      detallesByVenta.set(d.id_venta, []);
    }
