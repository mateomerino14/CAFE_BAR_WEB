...
export const getEmployeeSalesReportHandler = async (req, res) => {
  const { fechaInicio, fechaFin, empleadoId } = req.query;
  if (!fechaInicio || !fechaFin || empleadoId === undefined) return res.status(400).json({ message: 'Faltan parámetros' });
  try {
...
export const sendReportEmailHandler = async (req, res) => {
  ...
  if (!correoDestino || !htmlContent) return res.status(400).json({ message: 'Faltan datos para enviar el reporte' });
  try {
